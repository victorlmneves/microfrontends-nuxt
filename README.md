...
- **@module-federation/runtime 2.0.1** - Runtime API for dynamic remote loading (host only)
- **@module-federation/vite 1.10.0** - Vite plugin for building federated remotes (root package.json)
- **vite-plugin-top-level-await 1.6.0** - Enables top-level await (root package.json)
- **Turborepo** - Monorepo build orchestration
- **pnpm** - Fast, disk-space efficient package manager with workspace hoisting
...
Remotes use **@module-federation/vite** plugin (installed in root package.json, shared via workspace):
...
```typescript
const RemoteComponent = defineAsyncComponent(() => import('remoteName/ComponentName'))
```
