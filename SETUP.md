...
- Vue: 3.5.28
- Vite: 7.3.1
- @module-federation/runtime: 2.0.1 (host package.json only)
- @module-federation/vite: 1.10.0 (root package.json, shared via workspace)
- vite-plugin-top-level-await: 1.6.0 (root package.json, shared via workspace)
- pnpm: 8.15.0
...
> **Note**: All development and build tools (`@module-federation/vite`, `vite-plugin-top-level-await`, `@nuxt/devtools`, `typescript`, `vue-tsc`) are installed in the root package.json and imported via pnpm workspace hoisting. Remotes only declare their runtime dependencies.
...
2. Add `@module-federation/runtime` to host package.json
...
6. Move `@module-federation/vite` and `vite-plugin-top-level-await` to root package.json
