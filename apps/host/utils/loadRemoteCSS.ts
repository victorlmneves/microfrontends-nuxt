 ...
 * @param {string} remoteUrl - The base URL of the remote (e.g., 'http://localhost:3001')
 * @returns {Promise<void>} Promise that resolves when all CSS files are loaded
 */
export async function loadRemoteStyles(remoteUrl: string): Promise<void> {
    try {
        ...
        const cssUrls = [...new Set(matches)].map((filename) => `${remoteUrl}/_nuxt/${filename}`)
